...
struct Main{
	...
	#define MODES X(TELEOP)\
		...
		X(AUTO_PORTCULLIS) X(AUTO_PORTCULLIS_DONE) X(AUTO_CHEVALPOS) X(AUTO_CHEVALDROP) X(AUTO_CHEVALDRIVE) X(AUTO_CHEVALSTOW) X (AUTO_LBLS_CROSS_LB) X(AUTO_LBLS_CROSS_MU) X(AUTO_LBLS_SCORE_SEEK) X(AUTO_LBLS_SCORE_LOCATE) X(AUTO_LBLS_SCORE_CD) X(AUTO_LBLS_SCORE_EJECT) X(AUTO_LG_ROTATE) X(AUTO_LG_FORWARD)
